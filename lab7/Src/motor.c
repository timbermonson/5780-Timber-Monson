...
volatile uint8_t Kp = 11;            // Proportional gain
...
void encoder_init(void)
{
  ...
  TIM3->CCMR1 |=
      (TIM_CCMR1_CC1S_0 | TIM_CCMR1_CC2S_0); // TI1FP1 and TI2FP2 signals connected to CH1 and CH2
  TIM3->SMCR |=
      (TIM_SMCR_SMS_1 | TIM_SMCR_SMS_0); // Capture encoder on both rising and falling edges
  ...
  // (Could also cast unsigned register to signed number to get negative numbers if it rotates
  // backwards past zero
...
void ADC_init(void)
{
  ...
  while (ADC1->CR & ADC_CR_ADCAL)
    ; // Delay until calibration is complete
...
void PI_update(void)
{
  ...
  /* Run PI control loop
   *
   * Make sure to use the indicated variable names. This allows STMStudio to
  ...
  {                       // If the ADC has new data for us
